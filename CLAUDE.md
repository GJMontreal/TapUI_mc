...
|--------------|----------|--------------------------|
| I2C0 SDA     | GP4      | ST25DV16K SDA            |
| I2C0 SCL     | GP5      | ST25DV16K SCL            |
| GPO input    | GP15     | ST25DV16K GPO (active-low, PULL_UP) |
| WS2812 DIN   | GP28     | LED ring data in         |

- **NFC tag:** ST25DV16K — dual-interface EEPROM (NFC + I2C). iOS app writes NDEF; Pico reads/writes via I2C at address `0x53`.
...
`st25dv.py` caches the CC on first write so it is never corrupted. Writes are page-aligned (4 bytes) with a 5 ms inter-page delay as required by the ST25DV16K datasheet.
...
- The ST25DV16K I2C address (`0x53`) conflicts with some sensors. Verify with `mpremote exec "from machine import I2C, Pin; print(I2C(0, sda=Pin(4), scl=Pin(5)).scan())"` if the tag is not responding.
